// LFBuild.cpp : Defines the entry point for the console application.
// Note: Modified version of FBuild.cpp
//------------------------------------------------------------------------------
...
int Main( int argc, char * argv[] )
{
    ...
    // Note: @Modified: Change to LFBuild
    LFBuild fBuild( options );

    // load the dependency graph if available
    if ( !fBuild.LuaInitialize() )
    {
        if ( sharedData )
        {
